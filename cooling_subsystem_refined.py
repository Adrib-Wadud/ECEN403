...
tempRange_1 = range(1, 5, 1)  # 0-4 degrees from setting
tempRange_2 = range(5, 8, 1)  # 4-7 degrees from setting
tempRange_3 = range(8, 12, 1)  # 8-11 degrees from setting
tempRange_4 = range(12, 50, 1)  # 11-16 degrees from setting // Hotter than 16 degrees from setting sets fan to run at full speed.
currentHumidity = 100
...
humidityRange_1 = range(1, 6, 1)  # 0-5% away from cap
humidityRange_2 = range(6, 21, 1)  # 6-15% away from cap
...
def auto_control(currentTemp, tempSetting, currentHumidity, humidityCap):
    ...
    if tempDifference < 0:  # lower fanSpeed since your current temp is lower than your set temp
        ...
        if fan_Speed <= 1:  # Do nothing, you're close to set temp
            pass
        else:
            fan_Speed = fan_Speed - 1  # Lower fanspeed since you're getting close
    elif tempDifference in tempRange_2:
        if fan_Speed <= 2:
            fan_Speed = fan_Speed + 1  # Raise fanspeed to get closer to set temp
    ...
    if humidityDifference < 0:  # turn off humidifiers since it's too humid
        ...
        if humidifier_Intensity <= 1:  # Do nothing, you're close to set humidity
            pass
        else:
            humidifier_Intensity = humidifier_Intensity - 1  # Lower humidifierIntensity since you're getting close
    elif humidityDifference in humidityRange_2:
        if humidifier_Intensity <= 1:
            humidifier_Intensity = humidifier_Intensity + 1  # Raise humidifierIntensity to get closer to set humidity cap
        else:  # do nothing since you're on your way to set humidity cap
            pass
    elif humidityDifference in humidityRange_3:
        print('In most range')
        print(humidifier_Intensity)
        if humidifier_Intensity <= 2:
            print('H U')
            humidifier_Intensity = humidifier_Intensity + 1  # Raise humidifierIntensity to get closer to set humidity cap
            print(humidifier_Intensity)
        else:  # do nothing since you're on your way to set humidity cap
        ...
        humidifier_Intensity = 3  # max humidifierIntensity
    
    manual_control(fan_Speed, humidifier_Intensity)
